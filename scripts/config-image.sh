...
# Rewrite extlinux.conf matching the working upstream format:
# - fdtdir (U-Boot auto-selects the DTB), not fdt
# - rootwait (wait for root device), rw (not ro)
# - serial console ttyS2@1500000 + tty1 for HDMI
extlinux_conf="${chroot_dir}/boot/extlinux/extlinux.conf"
root_uuid=$(grep -oP 'UUID=\K[a-f0-9-]+' "${extlinux_conf}" 2>/dev/null | head -1)
if [ -n "${root_uuid}" ]; then
...
label l0
	menu label Ubuntu ${RELASE_VERSION} LTS ${kernel_version}
	linux /boot/vmlinuz-${kernel_version}
	initrd /boot/initrd.img-${kernel_version}
	fdtdir /lib/firmware/${kernel_version}/device-tree/
	append root=UUID=${root_uuid} rootwait rw console=ttyS2,1500000 console=tty1 cgroup_enable=cpuset cgroup_memory=1 cgroup_enable=memory

label l0r
	menu label Ubuntu ${RELASE_VERSION} LTS ${kernel_version} (rescue target)
	linux /boot/vmlinuz-${kernel_version}
	initrd /boot/initrd.img-${kernel_version}
	fdtdir /lib/firmware/${kernel_version}/device-tree/
	append root=UUID=${root_uuid} rootwait rw console=ttyS2,1500000 console=tty1 cgroup_enable=cpuset cgroup_memory=1 cgroup_enable=memory single
EOF
fi
